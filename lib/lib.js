var tp = {
	...
	run: function() {
		$('#output').text('');
		//try {
		  //Sk.importMain(true,tp.editor.getValue()); 
		
		
		Sk.misceval.asyncToPromise(function() {
			return Sk.importMainWithBody("<stdin>",false,tp.editor.getValue(),true);
		});
...
$(function() {
	...
	Sk.inputfun = function(prompt) {
		var p = new Promise(function(resolve, reject) {
			tp.printLine('<div id="raw_input_holder"><label for="raw_input">' + prompt + '</label><input type="text" name="raw_input" id="raw_input" value=""/><span id="raw_input_accept">OK</span></div>');
			$('#raw_input_accept').button().click(function() {
				var val = $('#raw_input').val();
				$('#raw_input_holder').remove();
				tp.printLine(prompt + " " + val + "\n");
				resolve(val);
			});
			$('#raw_input').textinput().focus();
		});
		return p;
	}
	// set defaults
