...
var tp = {
	...
	resetDebug: function() {
		...
		$('#btn_DebugStep,#btn_dbgStep').unbind().click(tp.onClick);
	...
	run: function(step) {
			...
			handlers["Sk.debug"] = function(susp) {
				try {				
					...
					var p = new Promise(function(resolve,reject){
						$('#btn_DebugStep,#btn_dbgStep').unbind().click(function() {
							resolve(susp.resume());
						});
						
						$('#btn_DebugReset,#btn_dbgReset').unbind().click(function() {
							tp.resetDebug();
		...
		Sk.misceval.callsimAsync(handlers, function() {
			...
			$('#btn_DebugStep,#btn_dgbStep').unbind().click(tp.onClick);
		});
	},
...
$(function() {
	...
	$('#chk_embed_output').on('change', function(e) {
		switch(e.currentTarget.value) {
			case 'show':
				$('#code_output_parent')[0].appendChild($('#output_sections')[0]);
			break;
			case 'hide':
				$('#output_holder')[0].appendChild($('#output_sections')[0]);
			break;
		}
	});
});
